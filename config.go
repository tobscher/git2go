...
func newConfigEntryFromC(centry *C.git_config_entry) (entry *ConfigEntry) {
	entry.Name = C.GoString(centry.name)
	entry.Value = C.GoString(centry.value)
	entry.Level = ConfigLevel(centry.level)
	return entry
}
